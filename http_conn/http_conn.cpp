...
// 网站的根目录
const char* doc_root = "/home/wjj/webserver2/resources";
...
http_conn::HTTP_CODE http_conn::parse_content( char* text ) {
    ...
    {
        //读指针大于消息体最后一个字符的索引  读完消息了 将最后一位置\0
...
http_conn::HTTP_CODE http_conn::process_read() {
    ...
    while (((m_check_state == CHECK_STATE_CONTENT) && (line_status == LINE_OK))
        ...
        // 获取当前行数据
        text = get_line();
        //m_checked_idx在phase_line中已经置于下一行开头 将m_start_line置于下一行开头
    ...
    while (1) {   
       ...
       temp=writev(m_sockfd,m_iv,m_iv_count);
        ...
        if (temp >= 0) {
            ...
            //偏移文件file iovec的指针
            newadd = m_bytes_have_send - m_write_idx;
        } else {
            //判断缓冲区是否满了
            if (errno == EAGAIN) {
